...
class ChenSortExampleState extends State<ChenSortExample> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('ChenSort performance comparison test'),
      ),
      body: ConstraintLayout().open(() {
        ...
        ListView.builder(
          ...
          itemBuilder: (_, index) {
            return Text('$index: ' + sortResult[index]);
          },
...
String compareSort(int numbers) {
  ...
  return 'ChenSort: $chenSortTimeUsage us, Quicksort: $quickSortTimeUsage us, ${percent.toStringAsFixed(2)}%(${(quickSortTimeUsage / chenSortTimeUsage).toStringAsFixed(1)}x) faster';
}
